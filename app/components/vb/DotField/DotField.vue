...
function setupCanvas() {
  ...
  // EMENDA LOCAL: `2` em tela Retina quadruplica a area rasterizada por quadro.
  // O ponto tem 1,5 px de raio e e desfocado por desenho — nao ha o que ganhar.
  const dpr = 1;
   ...
   * EMENDA LOCAL (25/08/2026): o original redesenhava a cada quadro. Num Mac

   * de 120 Hz isso desenhava ~6.200 pontos 120 vezes por segundo e sozinho

   * respondia por 54 pontos de CPU da home parada (medido: matar este laco

   * levava a tarefa do processo de 66,3% para 12,4%). A onda de fundo e lenta;

   * 30 quadros por segundo sao indistinguiveis e custam um quarto.
  ...
  function tick(agora: number) {
    ...
    if (!passouOIntervalo(ultimoQuadro, agora)) return;
